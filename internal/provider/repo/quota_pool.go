...
type QuotaPoolRepo interface {
	UpsertDepartmentQuotaPool(ctx context.Context, departmentID string, totalMicroCents int64) error
...
func (r *repo) UpsertDepartmentQuotaPool(ctx context.Context, departmentID string, totalMicroCents int64) error {
	_, err := r.pool.Exec(ctx, `
		INSERT INTO department_quota_pools (department_id, total_micro_cents, updated_at)
		VALUES ($1, $2, now())
		ON CONFLICT (department_id) DO UPDATE SET total_micro_cents = EXCLUDED.total_micro_cents, updated_at = now()`,
		departmentID, totalMicroCents,
	)
	return err
}
...
func (r *repo) GetDepartmentQuotaPoolTotal(ctx context.Context, departmentID string) (int64, error) {
	var total int64
	err := r.pool.QueryRow(ctx, `SELECT total_micro_cents FROM department_quota_pools WHERE department_id = $1`, departmentID).Scan(&total)
...
// the "spoken for" amount subtracted from total_micro_cents to get the live
...
func (r *repo) DepartmentQuotaPoolSpoken(ctx context.Context, departmentID string) (int64, error) {
	...
	err := r.pool.QueryRow(ctx, `
		SELECT COALESCE(SUM(vk.budget_micro_cents), 0)
