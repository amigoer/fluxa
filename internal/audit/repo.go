...
func (r *Repo) InsertCallLog(ctx context.Context, l CallLog) (CallLog, error) {
	err := r.pool.QueryRow(ctx, `
		...
		l.MemberID, l.VirtualKeyID, l.ProviderID, l.ModelID, l.RequestID, l.Status, l.LatencyMS, l.InputTokens, l.OutputTokens, l.CostCents,
...
func (r *Repo) ListCallLogs(ctx context.Context, limit int) ([]CallLog, error) {
	...
	for rows.Next() {
		var l CallLog
		if err := rows.Scan(&l.ID, &l.MemberID, &l.VirtualKeyID, &l.ProviderID, &l.ModelID, &l.RequestID, &l.Status, &l.LatencyMS, &l.InputTokens, &l.OutputTokens, &l.CostCents, &l.OccurredAt); err != nil {
			return nil, err
		}
		out = append(out, l)
	}
